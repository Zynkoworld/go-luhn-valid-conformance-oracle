#!/usr/bin/env python3
"""verify.py -- CI gate. Ujrafuttatja a go referenciat a kanonikus korpuszon, es exit 0 IFF minden
...
FUGGOSEG: go futtato a PATH-on (a bundle kodja nem python). Ha nincs, a gate NEM-MERT-tel bukik --
...
def main():
    ...
    if len({json.dumps(p["expect"], sort_keys=True) for p in probes}) < 2:
        ...
        return 1
    src = open(os.path.join(BASE, "oracle", "probe.go"), encoding="utf-8").read()
    inputs = [p["input"] for p in probes]
    r = subprocess.run(RUNNER + ["probe.go"], input=None, capture_output=True, text=True, timeout=60,
    ...
    print("luhn conformance oracle (go): probes=%d | agree=%d | disagree=%d"
